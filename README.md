...
- 新增轮询机制选项，默认开启（给用户更多选择，不再默认锁定开启轮询机制）
...
| `ech` / `echHost` / `echDns` | ECH 开关 / ECH 域名 / ECH DoH |
...
| `preferredDomains` / `preferredIPs` | 优选节点（域名 / 优选 API / IP，每行一个） |
...
| **节点配置** | ALPN（留空由客户端协商）/ TLS 控制（开启仅下发 TLS 节点）/ 节点数量控制（开启后限制下发节点总数）/ 轮询机制 |
| **ECH 配置** | ECH 开关 / 自定义 ECH 域名 / 自定义 ECH DoH |
...
- 测速结果实时结果，**同一 IP 自动去重**，可一键「加入优选」保存后随订阅下发
...
- **随机占比限制**：默认模式随机补足 ≤ 上限 20%，节点主体来自域名解析与内置优选池，保证开箱即用的节点质量
